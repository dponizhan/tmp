...
_xmlSerialize : function (name, object, prefix, context) {
	...
	return isc.Comm._xmlSerializeObject(name, data, context.objPath, 
                                        context.objRefs, prefix, isRoot, context.fieldCanBeFormula);
},
...
//		@return	(String)			serialized object as a string
//<
_xmlSerializeObject : function (name, object, objPath, objRefs, prefix, isRoot, canBeFormula) {
    ...
    // If a formula (technically UserSummary) is allowed as a field value and an object
    // is found with the "text" property, add UserSummary type wrapping so it can be
    // deserialized later.
    var innerTag = "";
    if (canBeFormula && object.text) {
        result += isc.Comm._xmlOpenTag("UserSummary", "UserSummary", null, null, null, isRoot);
        innerTag = isc.Comm._xmlCloseTag("UserSummary");
    }
