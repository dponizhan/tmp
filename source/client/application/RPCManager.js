...
// <p>
// Note that +link{group:sqlDataSource,SQL DataSources} don't add nulls to results for null
// values so this flag does nothing in that case.
//
// @visibility external
//<
...
// ---------------------------------------------------------------------------------------
    ...
    // <p>
    // Due to browser security restrictions, at most one request with a +link{group:upload, file
    ...
    _sendQueue : function (callback,prompt,URL,transaction) {
        ...
        if (request.clientLogContext) URL = this.addParamsToURL(URL, {isc_clctx: request.clientLogContext});
        else if (this.clientLogContext) URL = this.addParamsToURL(URL, {isc_clctx: this.clientLogContext});
    ...
    _isHttpResponseCommFailure : function (statusCode) {
        // unless a listed exception, 5xx status code is assumed to be a comm failure
        if (statusCode >= 500 && statusCode < 600) switch (statusCode) {
        ...
        // unless a listed exception, all other codes are assumed to be hard errors
        switch (statusCode) {
        case 404: // Not Found
        case 408: // Request Timeout
        case 420: // Enhance Your Calm
        case 430: // Request Header Fields Too Large
...
//<
    ...
    // Returns the data that should be sent to the +link{actionURL}.
    // <P>
    // In a manner analogous to +link{DataSource.transformRequest()}, this method allows you to
    // transform an +link{rpcRequest}, such as by adding +link{rpcRequest.httpHeaders,
    // HTTP headers}, to ensure proper handling on the server.
    // <smartgwt><P>
    // This is not an override point.  See
    // {@link RPCManager#customizeTransforms(RequestTransformer)}.</smartgwt>
    //
    // @param rpcRequest (RPCRequest) the RPCRequest being processed
    // @return (Any) data to be sent to the actionURL
    // @visibility external
    //<
    ...
    // first project specified in the projectNames argument to +link{rpcManager.loadProject}.
    //
    // @visibility external
    //<
    ...
    // Loads projects using the +link{group:servletDetails,ProjectLoaderServlet}, reachable at 
    // +link{rpcManager.projectLoaderURL}, and fires the given callback after screens have been  
    // +link{rpcManager.cacheScreens, cached}.
    //
    // @param projectNames (String) Comma separated string containing the names of project/s to load.
    // @param callback     (Function) callback for notification of completion of project/s loaded and screens cached.
    ...
    loadProject : function (projectNames, callback, settings) {
        ...
        // cacheScreens will fire the given callback when all screens (if any) are cached
        this._screensCachedCallback = callback;
    ...
    cacheScreens : function(screenName, callback, locale, requestProperties) {
        ...
        request.callback = function (rpcResponse, data, rpcRequest) {
            ...
            isc.Class.globalEvalWithCapture(data, function (globals, error) {
                ...
                if (_this._screensCachedCallback) {
                    _this.fireCallback(_this._screensCachedCallback, "data,rpcResponse", [json, rpcResponse]);
                }
...
isc.RPCManager.addClassProperties({
    // bind relogin field names
    loginUsernameFieldName:  window.isc_user && window.isc_user.loginUsernameFieldName ? 
        window.isc_user.loginUsernameFieldName : "username",
    loginPasswordFieldName: window.isc_user && window.isc_user.loginPasswordFieldName ? 
        window.isc_user.loginPasswordFieldName : "password",
    lockReloginToCurrentUser: window.isc_user && window.isc_user.lockReloginToCurrentUser,

    loginUsernameFieldTitle: "Username or email",
    loginPasswordFieldTitle: "Password"
});

isc.RPCManager.addClassProperties({
    loginFailureFieldDefaults: {
        name: "loginFailure",
        type: "blurb",
        defaultValue: "Please check your Username/Email and Password.",
        visible: false,
        colSpan: 2
    },
    loginUsernameFieldDefaults: {
        name: isc.RPCManager.loginUsernameFieldName, 
        type: "textItem",
        title: isc.RPCManager.loginUsernameFieldTitle, 
        canEdit: isc.RPCManager.lockReloginToCurrentUser ? false : true,
        defaultValue: isc.RPCManager.lockReloginToCurrentUser ? window.isc_user.isc_username : null,
        keyPress : function (item, form, keyName) {
            if (keyName == "Enter") {
                form.focusInItem(this.loginPasswordFieldName);
                return false;
            }
        }
    },
    loginPasswordFieldDefaults: {
        name: isc.RPCManager.loginPasswordFieldName, 
        title: isc.RPCManager.loginPasswordFieldTitle, 
        type: "password", 
        keyPress : function (item, form, keyName) {
            if (keyName == "Enter") {
...
isc.RPCManager.addClassProperties({
    ...
    loginFormDefaults: {
        ...
        ignoreProgramErrors: isc.params.ignoreProgramErrors == 1,
        fields : [
...
// </ul>
// 
// If you need to do a <b>remove</b> with criteria rather than a primary key, the recommended
// approach is to do it as a <b>custom</b> operation so the intent is clear.  You can also do it
// as a <b>remove</b>, but just use +link{dSRequest.oldValues} as the source for whatever
// criteria values you need.  That covers everything except +link{AdvancedCriteria}.
//
...
// <code>servlet.sendStackTraceToClient</code> (boolean) exception stacktrace can also be included.
//
