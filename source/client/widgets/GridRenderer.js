...
shouldShowAllColumns : function () {
    ...
    // If we're auto-fitting horizontally render out all cols
    if (this.autoFitData = "both" || this.autoFitData == "horizontal") return true;
    return false;
},
...
getDrawArea : function (colNum, scrollTop) {
    ...
    if (this.showAllRows || showAllCells) {
        ...
        // If our length is not known, ensure we draw enough rows to satisfy drawAllMaxCells
        // by default.
        
        var range = rowArr[1] - rowArr[0],
            maxCells = this.getDrawAllMaxCells(),
            drawAllMaxRows = 0;
        if (totalCols > 0 && maxCells && !this.lengthIsKnown()) {
            drawAllMaxRows = Math.floor(maxCells / totalCols);
        }

        
        if (rowArr[1] < drawAllMaxRows) {
            startRow = 0;
            endRow = drawAllMaxRows-1;
        } else {
            startRow = rowArr[0];
            endRow = rowArr[1];
            // just for logging
...
_rowAnimationComplete : function () {
    ...
    if (isc.screenReader) {
        ...
        this._putNativeFocusInRow(this.getNativeFocusRow(), this.getNativeFocusColumn(), focusCanvas != null && focusCanvas !== this);
    }
...
_updateVirtualScrolling : function (setFirstAndLastDrawnRow) {
    var drawRect = this._getTableHTMLDrawArea(),
        grid = this.grid
    ;
            ...
            (newIsVirtualScrolling || this._canStopVirtualScrolling()))
        {
            ...
            if (this._isVirtualScrolling) { 
                ...
                if (this.isDrawn()) {
                    ...
                    drawRect = this._getTableHTMLDrawArea();
                }
            } else {
    ...
    if (setFirstAndLastDrawnRow) {
        ...
        this._lastDrawnRow = drawRect[1] - 1;
    }
    ...
    if (isc.screenReader) {
        ...
        if (reason == "focus on mousedown") {
            ...
            if (eventRow != null && !this.isEmpty()) {
                ...
                if (eventRow >= 0) {
                    this._putNativeFocusInRow(eventRow, this.getEventColumn(), true);
                }
            }
        }
    }
...
_putNativeFocusInRow : function (rowNum, colNum, suppressElementFocus) {
    ...
    // colNum is always passed around, but ignored in getFocusHandle() unless this.canSelectCells is true
    this._nativeFocusColumn = colNum;
...
getFocusHandle : function () {
    if (isc.screenReader && !this._skipReturningNativeFocusRow) {
        var rowNum = this.getNativeFocusRow(), 
            colNum = this.canSelectCells ? this.getNativeFocusColumn() : null,
            element = this.getTableElement(rowNum, colNum);
...
addEmbeddedComponent : function (component, record, rowNum, colNum, position, suppressRedraw) {
    ...
    if (moveOnly) {
        this.placeEmbeddedComponent(component, suppressRedraw);
...
placeEmbeddedComponent : function (component, suppressRedraw) {
    ...
    if (position == this._$within) {
        ...
        if (this.grid.indentRecordComponents && component.snapOffsetLeft) {
            width -= component.snapOffsetLeft;
        }
...
//    if (position != this._$within) {
        ...
        if (needsResize && (expectedRowHeight > this.getRowSize(rowNum))) {
            ...
            // Skip this if passed the suppressRedraw flag - assume the parent is
            // refreshing the row or redrawing the body
            if (!suppressRedraw) {
...
removeEmbeddedComponent : function (record, component, suppressDeparent, suppressRedraw) {
    // support specifying rowNum instead
    ...
    // suppress deparent - used when an embedded component is just being shifted to another record
    if (suppressDeparent) {
...
getNativeFocusColumn : function () {
    var colNum = this._nativeFocusColumn;
    if (colNum == null || colNum < this._firstDrawnCol || colNum > this._lastDrawnCol) colNum = 0
    return colNum;
}, 
...
_getViewportFillRows : function (scrollTop) {
    ...
    if (this.autoFitData == "both" || this.autoFitData == "vertical") {
        ...
        // We need to fill the space from the top of the first record (may be slightly above
        // the viewport top) to the bottom of the viewport
        var topOffset = scrollTop - (firstVisible * this.cellHeight);
        nRecords = Math.ceil((topOffset + viewportHeight) / this.cellHeight);
    }
...
_getVisibleRows : function () {
    ...
    var rows = [
        ...
        // passing the parameter to 'getInnerHeight()' ensures that if
        // we're overflow:visible, we check our drawn area rather than our
        // specified area
        this.getEventRow(scrollTop + this.getInnerHeight(true))
    ];
...
//> @attr gridRenderer.showHoverOnDisabledCells (boolean : false : IRWA)
// If +link{showHover} is true, should cell hover HTML be displayed on disabled cells?
// @visibility external
//<
showHoverOnDisabledCells:false,
...
mouseMove : function (arg1, arg2) {
    ...
    var validRowCol = (rowNum >= 0 && colNum >= 0),
        ...
        hasNewCell = (validRowCol && requireRecord && 
                        (this.showHoverOnDisabledCells || this.cellIsEnabled(rowNum, colNum, record)));
        ...
        if (field) {
            ...
            for (var hoverProp in this._hoverPropertyMap) {
                ...
                result[hoverProp] = field[widgetProp] != null
                                        ? field[widgetProp] 
                                        : (this.grid[widgetProp] != null 
                                                ? this.grid[widgetProp] 
                                                : this[widgetProp]);
            }
            return result;
        }
...
_cellSelectionChanged : function (cellList) {
    ...
    if (this.grid && this.grid.selectionAppearance == "checkbox" && this.grid.canSelectCells) {
