...
_destroyChildren : function () {
    if (this._titleLabel) this._titleLabel.destroy();
    this._track.destroy();
    this._thumb.destroy();
    this._valueLabel.destroy();
    if (this._valueLabelLayoutContainer) {
        this._valueLabelLayoutContainer.destroy();
        delete this._valueLabelLayoutContainer;
    }
    if (this._valueLabelLayout) {
        this._valueLabelLayout.destroy();
        delete this._valueLabelLayout;
    }
...
setUpSize : function () {
	...
	this._usableLength = this.length - thumbThinWidth - (this.getHPadding()/2);
	if (this.numValues) {
...
_updateValueLabel : function () {
    ...
    // if there's a labelLayout, shink it so it can overflow with the new, 
    ...
    if (this.vertical) {
        var newTop = parseInt((thumb.getTop() + thumb.getHeight()/2) - layoutContainer.getHeight() / 2);
        layoutContainer.setTop(newTop-1);
    } else {
        ...
        ;
        if (newLeft + labelWidth > this.getInnerWidth()-1) {
            // clamp the valueLabel to the Slider rect
            newLeft = this.getInnerWidth() - labelWidth - 1;
        }
