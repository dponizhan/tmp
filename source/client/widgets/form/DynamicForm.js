...
isc.DynamicForm.addClassProperties({
    ...
    _defaultItemHoverHTMLImpl : function (item) {
        ...
        var prompt = (item.isDisabled() ? item.disabledHover : null) || 
...
setValues : function (newData, initTime, skipRememberValues, skipRuleContextChange) {
    ...
    if (this.ruleScope || this.isRuleScope) {
        ...
        if (ds && isc.isA.DataSource(ds) && this._populateSharedRuleContext != false) {
            this.provideRuleContext(ds.getID(), values,
                this, null, !(!hasStableID && skipRememberValues) || skipRuleContextChange);
        }
        if (hasStableID) {
            this.provideRuleContext(this.getLocalId() + ".values", values,
                this, null, !skipRememberValues || skipRuleContextChange);
        ...
        (fieldName || (field && field.name)))
    {
        ...
        if (ds && isc.isA.DataSource(ds) && this._populateSharedRuleContext != false) {
            this.provideRuleContext(ds.getID() + "." + fieldName, value,
                this, null, hasStableID || this._setValuesPending);
        }
        if (hasStableID) {
    ...
    for (var i = 0; i < items.length; i++) {
                ...
                (item.textFormula && !isc.isA.emptyObject(item.textFormula)))
        {
            ...
            if (badProperties) {
                ...
                // Cache locator to be used when removing rule(s)
                item._ruleLocator = locator;
            }
        }
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            if (item.icons) {
                ...
                for (var j = 0; j < icons.length; j++) {
                    ...
                    delete icon._ruleLocator;
                }
            }
            delete item._ruleLocator;
