...
isc.ColorItem.addMethods({
    ...
    updateValue : function () {
        ...
        var oldValue = this._value,
            // if there's no element, eg showInputElement is false, call getValue() instead
            value = this.hasElement() ? this.getElementValue() : this.getValue();
        ...
        // if the new value is an empty string, make it null instead, so the value is cleared
        if (value == "") value = null;

        // If the user entered an invalid color just refuse to accept it.
        if (value != null && !isc.isA.color(value) && value != "transparent") {
        ...
        if (this.showPickerIcon && this._value != oldValue) {
            ...
            if (value == "transparent") {
                this.setIconCustomState(pickerIcon, value);
    ...
    _pickerColorSelected : function (color, opacity) {
        ...
        if (this.picker && this.picker._currentPickMode == 'complex') {
            // if opacity isn't one of null, 0 or 100, append the opacity value to the color, 
            // as a hex number - but only in complex mode, where there is a UI present to 
            // affect opacity - in simple mode, you're just picking the color from the swatch
            if (opacity != null) {
                if (opacity == 0) color += "00";
                else if (opacity != 100) {
                    // only convert to hex8 if the opacity is not 100%
                    var oHex = isc.ColorUtils.decToHex(Math.round((opacity/100)*255));
                    color += oHex;
                }
            }
        }
