...
isc.TextItem.addMethods({
    ...
    //> @method textItem.getValueAsInteger()
        ...
        if (this.mask) {
            ...
            if (!selectOnFocus) {
                ...
                if (isc.Browser.isFirefox) {
                    // use delayCall() to confound a Firefox behavior which selects the whole value
                    // automatically - causes a flash right now
                    if (!this._delayedSelect) {
                        this._delayedSelect = this.delayCall("_delayed_setSelection", [begin, end]); //, 20);
                    }
                } else {
                    this.setSelectionRange(begin, end);
                }
            } else {
                // if selecting on focus, do so via delayCall
                if (!this._delayedSelect) {
            ...
            if (keyName == "Backspace" || keyName == "Delete") {
                    ...
                    (isSafari && this._lastSelection))
                {
                    ...
                    if (keyName == "Backspace") {
                        ...
                        if (!isc.Browser.isFirefox) shiftPos--;
                        if (shiftPos >= 0) {
                            if (this.maskOverwriteMode) {
