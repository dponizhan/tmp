...
isc.SpinnerItem.addMethods({
    ...
    getTextBoxWidth : function (value) {
        ...
        // text box width to accommodate them while keeping the control table width looking
        // right.
        if (this._writeSpinnersIntoControlTable()) {
    ...
    getLogicalTextBoxWidth : function () {
        var width = this.Super("getLogicalTextBoxWidth", arguments);
        // If we're writing spinners into the control table we want to reduce the
        // text box width to accommodate them while keeping the control table width looking
        // right.
        if (this._writeSpinnersIntoControlTable()) {
    ...
    handleKeyPress : function (event, eventInfo) {
        ...
        if (this.changeOnKeypress) {
            var eValue = this._changeValue || keyName;
            if (eValue == "-") {
                // if the new value is just "-", set the value to zero and flag as negative -
                // required because 0 == -0  
                this._changeValue = 0;
                this.isNegative = true;
                return true;
            } else if (eValue == isc.NumberUtil.decimalSymbol) {
                // if the new value is just the locale decimalSymbol, set the value to "0.0" 
                // and select the second zero
    ...
    updateValue : function () {
        ...
        if (value != null && (isNaN(value) == true || !this.isInRange(value))) {
