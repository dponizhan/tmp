...
isc.DetailViewer.addProperties({
    ...
    // not <smartclient>observe</smartclient><smartgwt>detect</smartgwt> changes to the data array 
    // (in other words they will not automatically
    ...
    //> @attr   detailViewerField.imageURLSuffix (String : null : IRWA)
    // If this field has type set to <code>"image"</code>, the value
	...
	for (var i = 0; i < valueList.getLength(); i++) {
        ...
        if (field.type == "image") {
            ...
            if (src == this._resolveEmptyDisplayValue(field)) {
                ...
                formattedValue = this.formatValueAsImage(src, field, record);
            }
...
getCellValue : function (record, field) {
    ...
    if (field.formatCellValue == null && this.formatCellValue) {
        ...
        if (value == null || isc.is.emptyString(value)) {
            ...
            if (escapeHTML) value = value.asHTML(null, field.skipLineBreaks);
        }
    }
